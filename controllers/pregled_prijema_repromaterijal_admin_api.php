<?php
class Pregled_Prijema_Repromaterijal_Admin_Api extends Controller
{
    ...
    public function get_search_good_type_admin()
    {
        ...
        header('Content-Type: application/json');
            $sql = "SELECT
                    ...
                    WHERE input_repromaterijal.sort_of_goods_id= :sort_of_goods_id
    ...
    public function get_search_good_client_admin()
    {
        ...
        $wearehouse_upit = isset($_GET['wearehouse_id']) || !($_GET['wearehouse_id'] == NULL) ? 'input_records.wearehouse_id= :wearehouse_id AND ' : '';
        ...
        $sql = "SELECT
                ...
                INNER JOIN input_repromaterijal ON (input_repromaterijal.input_id = input_records.input_id)
                INNER JOIN clients ON (clients.client_id = input_records.client_id)
                WHERE  ".$wearehouse_upit." input_repromaterijal.sort_of_goods_id= :sort_of_goods_id
                AND input_repromaterijal.type_of_goods_id= :type_of_goods_id
                AND input_repromaterijal.goods_id= :goods_id
                AND input_records.exit_date !='0000-00-00 00:00:00'
                AND  input_records.stornirano='n'
    ...
    public function get_search_prijem()
    {
        ...
        $type_of_goods_id = isset($_GET['type_of_goods_id']) ? 'input_repromaterijal.type_of_goods_id= :type_of_goods_id' : '';
        $goods_id = isset($_GET['goods_id']) ? ' AND input_repromaterijal.goods_id= :goods_id' : '';
        $wearehouse = isset($_GET['wearehouse_id']) || !($_GET['wearehouse_id'] == NULL) ? 'AND input_records.wearehouse_id= :wearehouse_id' : '';
        $client_id = isset($_GET['client_id']) || !($_GET['client_id'] == NULL)  ? ' AND input_records.client_id= :client_id' : '';
        $datum_od = isset($_GET['datum_od']) || !($_GET['datum_od'] == NULL) ? ' AND DATE(input_records.input_date)>= :datum_od' : '';
        $datum_do = isset($_GET['datum_do']) || !($_GET['datum_do'] == NULL) ? ' AND DATE(input_records.input_date)<= :datum_do' : '';
        ...
        $sql = "SELECT
                ...
                AND input_records.exit_date !='0000-00-00 00:00:00'
                AND input_repromaterijal.sort_of_goods_id='2'
                AND input_records.stornirano='n' ORDER BY input_records.input_id";
   ...
   /* public function storniraj_dokument($input_id)
    ...
    public function getPrijem($session_id=null){
        ...
        header('Content-Type: application/json');
            ...
            $result = $this->model->get_values($sql, array(":sort_of_goods_id"=>"1", ":input_id"=>$input_id));
            $result = $result[0];
    ...
    public function select_last_input(){
        ...
        if( $check_session['login'] == 1){
            $sql='SELECT
                ...
                WHERE exit_date !="0000-00-00 00:00:00"
                AND input_repromaterijal.sort_of_goods_id != :sort_of_goods_id
                ORDER BY input_records.input_id DESC LIMIT 1';
