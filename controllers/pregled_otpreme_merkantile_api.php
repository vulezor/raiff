...
class Pregled_Otpreme_Merkantile_Api extends Controller{
    ...
    public function get_search_otprema($ses){
        ...
        $type_of_goods_id = isset($_GET['type_of_goods_id']) ? ' AND output_merkantila.type_of_goods_id= :type_of_goods_id' : '';
        $goods_id = isset($_GET['goods_id']) ? ' AND output_merkantila.goods_id= :goods_id' : '';
       ...
       /* print_r($params).'<br />';
        ...
        if( $check_session['login'] == 1){
            ...
            $sql = "SELECT
                ...
                AND output_merkantila.sort_of_goods_id='1'
    ...
    public function get_search_otprema_admin(){
        //type_of_goods_id=1&goods_id=94&client_id=1&datum_od=01.01.2016&datum_do=03.01.2016
        $check_session = $this->check_logedIn_admin(); //checking if session exists
        //$wearehouse = 'output_records.wearehouse_id= :wearehouse_id';
        $type_of_goods_id = isset($_GET['type_of_goods_id']) ? 'output_merkantila.type_of_goods_id= :type_of_goods_id' : '';
        $goods_id = isset($_GET['goods_id']) ? ' AND output_merkantila.goods_id= :goods_id' : '';
        $wearehouse_id = isset($_GET['wearehouse_id']) ? ' AND output_records.wearehouse_id= :wearehouse_id' : '';
        $client_id = isset($_GET['client_id']) ? ' AND output_records.client_id= :client_id' : '';
        $datum_od = isset($_GET['datum_od']) ? ' AND DATE(output_records.output_date)>= :datum_od' : '';
        $datum_do = isset($_GET['datum_do']) ? ' AND DATE(output_records.output_date)<= :datum_do' : '';
        $statement = $type_of_goods_id." ".$goods_id." ".$wearehouse_id." ".$client_id." ".$datum_od." ".$datum_do;
        $params = array();
    ...
    public function get_search_good_type(){
        ...
        if( $check_session['login'] == 1){
            $sql = "SELECT output_merkantila.type_of_goods_id, type_of_goods.goods_type FROM output_records
                    INNER JOIN output_merkantila ON (output_records.output_id = output_merkantila.output_id)
                    INNER JOIN type_of_goods ON (type_of_goods.type_of_goods_id = output_merkantila.type_of_goods_id)
                    WHERE output_records.wearehouse_id= :wearehouse_id AND output_merkantila.sort_of_goods_id= :sort_of_goods_id
                    AND output_records.exit_date !='0000-00-00 00:00:00' AND output_records.stornirano='n' GROUP BY output_merkantila.type_of_goods_id";
    ...
    public function get_search_good_type_admin(){
        ...
        if( $check_session['login'] == 1){
            $sql = "SELECT output_merkantila.type_of_goods_id, type_of_goods.goods_type FROM output_records
                    INNER JOIN output_merkantila ON (output_records.output_id = output_merkantila.output_id)
                    INNER JOIN type_of_goods ON (type_of_goods.type_of_goods_id = output_merkantila.type_of_goods_id)
                    WHERE  output_merkantila.sort_of_goods_id= :sort_of_goods_id
                    AND output_records.exit_date !='0000-00-00 00:00:00' AND output_records.stornirano='n' GROUP BY output_merkantila.type_of_goods_id";
    ...
    public function get_search_good_client_admin()
    {
        ...
        if(isset($_GET['wearehouse_id'])){
            $wearehouse = 'output_records.wearehouse_id= :wearehouse_id AND ';
        }
        header('Content-Type: application/json');
        if ($check_session['login'] == 1) {
            $sql = "SELECT clients.client_id, clients.firm_name FROM output_records
                    INNER JOIN output_merkantila ON (output_records.output_id = output_merkantila.output_id)
                    INNER JOIN clients ON (clients.client_id = output_records.client_id)
                    WHERE ".$wearehouse." output_merkantila.sort_of_goods_id= :sort_of_goods_id
                    AND output_merkantila.type_of_goods_id= :type_of_goods_id
                    AND output_merkantila.goods_id= :goods_id
                    AND output_records.exit_date !='0000-00-00 00:00:00'
                    AND  output_records.stornirano='n'
    ...
    public function get_search_good_wearehouses_admin(){
        ...
        if ($check_session['login'] == 1) {
            ...
            $sqlc = "SELECT wearehouses.wearehouse_id, wearehouses.wearehouse_name FROM output_records
                    ...
                    AND output_records.exit_date !='0000-00-00 00:00:00'
                    AND  output_records.stornirano='n'
                    GROUP BY wearehouses.wearehouse_id";
    ...
    public function get_search_otprema_total_admin(){
        //type_of_goods_id=1&goods_id=94&client_id=1&datum_od=01.01.2016&datum_do=03.01.2016
        $check_session = $this->check_logedIn_admin(); //checking if session exists
        //$wearehouse = 'output_records.wearehouse_id= :wearehouse_id';
       /* echo'<pre>';
        ...
        $type_of_goods_id = isset($_GET['type_of_goods_id']) ? 'output_merkantila.type_of_goods_id= :type_of_goods_id' : '';
        $goods_id = isset($_GET['goods_id']) ? ' AND output_merkantila.goods_id= :goods_id' : '';
        $wearehouse_id = isset($_GET['wearehouse_id']) && !empty($_GET['wearehouse_id']) ? ' AND output_records.wearehouse_id= :wearehouse_id' : '';
        $client_id = isset($_GET['client_id']) && !empty($_GET['client_id']) ? ' AND output_records.client_id= :client_id' : '';
        $datum_od = isset($_GET['datum_od']) && !empty($_GET['datum_od']) ? ' AND DATE(output_records.output_date)>= :datum_od' : '';
        $datum_do = isset($_GET['datum_do']) && !empty($_GET['datum_do']) ? ' AND DATE(output_records.output_date)<= :datum_do' : '';
        $statement = $type_of_goods_id." ".$goods_id." ".$wearehouse_id." ".$client_id." ".$datum_od." ".$datum_do;
        $params = array();
        ...
        foreach($_GET as $key => $value){
            ...
            if($key!=='session_id' || $key!=='url' ){
                if ($key==="type_of_goods_id" && !empty($value)){
                    $params[':type_of_goods_id'] = $value;
                } elseif ($key==="goods_id" && !empty($value)){
                    $params[':goods_id'] = $value;
                }elseif ( $key==="wearehouse_id" && !empty($value) ){
                      $params[':wearehouse_id'] = $value;
                }elseif ($key === "client_id" && !empty($value) ){
                    $params[':client_id'] = $value;
                } elseif($key==="datum_od" && !empty($value) ){
                    $params[':datum_od'] = date('Y-m-d', strtotime($value));
                } elseif ($key==="datum_do" && !empty($value)) {
                    $params[':datum_do'] = date('Y-m-d', strtotime($value));
                }
            }
        }
       /* echo'<pre>';
      //  echo gettype($datum_od).'<br />';
         print_r($params).'<br />';
         echo $statement;return false;*/
    ...
    public function printExcel($get, $result, $suma){
        ...
        $xml = new ExcelWriterXML('Pregled otpreme merkantile .xls');
        ...
        $sheet->writeString(1,1,'IZVOD STANJA OTPREME '.$good_name.' U MAGACINU '.strtoupper (Session::get('wearehouse_name')).''.$client_name.''.$datum_od ,$format4);
    ...
    public function get_goods_type(){
        ...
        if ($check_session['login'] == 1) {
            Ajax::ajaxCheck();
            header('Content-Type: application/json');
            $sql = "SELECT * FROM type_of_goods WHERE goods_type='kukuruz suseni' || goods_type='psenica tel-kel' || goods_type='kukuruz tel-kel' || goods_type='kukuruz' || goods_type='psenica' || goods_type='suncokret' || goods_type='soja' || goods_type='jecam' || goods_type='uljana repica' || goods_type='sacma'";
            $result = $this->model->get_values($sql,$id=null);
            echo json_encode($result, JSON_NUMERIC_CHECK);
        } else {
            echo json_encode(array('logedIn' => 0), JSON_NUMERIC_CHECK);
    ...
    public function select_last_output(){
        ...
        if( $check_session['login'] == 1){
            $sql='SELECT
                ...
                INNER JOIN goods ON (goods.goods_id = output_merkantila.goods_id)
                INNER JOIN type_of_goods ON (type_of_goods.type_of_goods_id = output_merkantila.type_of_goods_id)
                WHERE exit_date !="0000-00-00 00:00:00" AND output_merkantila.sort_of_goods_id= :sort_of_goods_id
    ...
    public function getEnabledDays(){
        Ajax::ajaxCheck();
        $sql = "SELECT DATE_FORMAT(DATE(output_records.output_date),'%e-%c-%Y') FROM output_records
                INNER JOIN output_merkantila ON( output_merkantila.output_id = output_records.output_id)
                WHERE output_merkantila.sort_of_goods_id= :sort_of_goods_id AND output_records.stornirano='n'   GROUP BY DATE(output_records.output_date)";
        $obj = array(':sort_of_goods_id'=>'1');
        $result = $this->model->get_values($sql,$obj);
        echo json_encode($result);
    }
    ...
    public function enable_days(){
        ...
        if( $check_session['login'] == 1){
            $sql = "SELECT DATE_FORMAT(DATE(output_records.output_date),'%e-%c-%Y') as datum FROM output_records
                    INNER JOIN output_merkantila ON (output_merkantila.output_id = output_records.output_id)
                    WHERE output_merkantila.sort_of_goods_id= :sort_of_goods_id   GROUP BY DATE(output_records.output_date)";
            $obj = array(':sort_of_goods_id'=>'1');
            $result = $this->model->get_values($sql,$obj);
            echo json_encode($result, JSON_NUMERIC_CHECK);
    ...
    public function otprema_merkantila(){
        ...
        if( $check_session['login'] == 1){
            ...
            $output_id = $this->model->set_values('output_records',  $output_info); //vraca id prijemnice
    ...
    public function get_search_type(){
        ...
        $check_session = $this->check_logedIn_admin(); //checking if session exists
        if( $check_session['login'] == 1){
            $output_date = date('Y-m-d', strtotime($data->datumPrijema));
            $sql = "SELECT type_of_goods.type_of_goods_id, type_of_goods.goods_type FROM output_records
                    INNER JOIN output_merkantila ON (output_merkantila.output_id = output_records.output_id)
                    INNER JOIN type_of_goods ON (type_of_goods.type_of_goods_id = output_merkantila.type_of_goods_id)
                    WHERE output_merkantila.sort_of_goods_id= :sort_of_goods_id
                    AND DATE(output_records.output_date)= :output_date
                    AND output_records.stornirano='n'
                    GROUP BY output_merkantila.type_of_goods_id";
            $obj = array(':sort_of_goods_id'=>'1', ':output_date'=>$output_date);
            $result = $this->model->get_values($sql,$obj);
            echo json_encode($result, JSON_NUMERIC_CHECK);
        } else {
            echo json_encode(array('logedIn'=>0), JSON_NUMERIC_CHECK);
    ...
    public function get_search_good_name_prijem(){
        ...
        if( $check_session['login'] == 1){
            ...
            $sql = "SELECT goods.goods_id, goods.goods_name  FROM output_records
                    ...
                    AND output_records.stornirano='n'
                    GROUP BY goods_id";
            $obj = array(':sort_of_goods_id'=>'1', ':output_date'=>$output_date, ':type_of_goods_id'=>$type_of_goods_id);
            $result = $this->model->get_values($sql,$obj);
            echo json_encode($result, JSON_NUMERIC_CHECK);
    ...
    public function get_search_otpremnica(){
        Ajax::ajaxCheck();
        $data = json_decode(file_get_contents("php://input"));//take data from json object
      ...
      /* echo'<pre>';
        ...
        if( $check_session['login'] == 1){
            ...
            $obj = array(':sort_of_goods_id'=>'1', ':output_date'=>$output_date, ':type_of_goods_id'=>$type_of_goods_id, ':goods_id'=>$goods_id, ':stornirano'=>"n", ":exit_date"=>"0000-00-00 00:00:00");
            $result = $this->model->get_values($sql,$obj);
            echo json_encode($result, JSON_NUMERIC_CHECK);
    ...
    public function select_odabranu_otpremu(){
        Ajax::ajaxCheck();
        $data = json_decode(file_get_contents("php://input"));//take data from json object
        /*echo'<pre>';
        print_r($data);*/
       // print_r($data->inputs[0]->output_id);return false;
        $check_session = $this->check_logedIn_admin(); //checking if session exists
        header('Content-Type: application/json');
        if( $check_session['login'] == 1){
            $sql='SELECT
                    ...
                    AND output_records.stornirano= :stornirano
                    AND output_records.exit_date!= :exit_date';
            $result = $this->model->get_values($sql, array(":sort_of_goods_id"=>"1", ":output_id"=>$data->output_id, ":stornirano"=>"n", ":exit_date"=>"0000-00-00 00:00:00"));
            echo json_encode($result);
