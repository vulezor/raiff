<?php
class Pregled_Prijema_Repromaterijal_Api extends Controller
{
    ...
    public function get_search_prijem($ses)
    {
        ...
        $type_of_goods_id = isset($_GET['type_of_goods_id']) ? ' AND input_repromaterijal.type_of_goods_id= :type_of_goods_id' : '';
        $goods_id = isset($_GET['goods_id']) ? ' AND input_repromaterijal.goods_id= :goods_id' : '';
        ...
        if ($check_session['login'] == 1) {
            ...
            $sql = "SELECT
                ...
                WHERE  " . $wearehouse . " " . $type_of_goods_id . " " . $goods_id . " " . $client_id . " " . $datum_od . " " . $datum_do . "  AND input_records.exit_date !='0000-00-00 00:00:00' AND input_repromaterijal.sort_of_goods_id='2' AND input_records.stornirano='n' ORDER BY input_records.input_id";
    ...
    public function get_search_good_type()
    {
        ...
        if ($check_session['login'] == 1) {
            $sql = "SELECT
                    input_repromaterijal.type_of_goods_id,
                    ...
                    GROUP BY input_repromaterijal.type_of_goods_id";
            $obj = array(':wearehouse_id' => Session::get('wearehouse_id'), ':sort_of_goods_id' => '2');
    ...
    public function get_search_good_name()
    {
        ...
        if ($check_session['login'] == 1) {
            $sql = "SELECT
                    ...
                    AND input_repromaterijal.sort_of_goods_id= :sort_of_goods_id
                    AND input_records.exit_date !='0000-00-00 00:00:00'
                    AND input_repromaterijal.type_of_goods_id= :type_of_goods_id
                    AND input_records.stornirano='n'
                    GROUP BY goods.goods_id";
            $obj = array(':wearehouse_id' => Session::get('wearehouse_id'), ':sort_of_goods_id' => '2', ':type_of_goods_id' => $type_of_goods_id);
            $result = $this->model->get_values($sql, $obj);
            echo json_encode($result, JSON_NUMERIC_CHECK);
        } else {
            echo json_encode(array('logedIn' => 0), JSON_NUMERIC_CHECK);
    ...
    public function get_search_good_client()
    {
        ...
        if ($check_session['login'] == 1) {
            $sql = "SELECT
                    ...
                    AND input_repromaterijal.sort_of_goods_id= :sort_of_goods_id
                    AND input_repromaterijal.type_of_goods_id= :type_of_goods_id
                    AND input_repromaterijal.goods_id= :goods_id
                    AND input_records.exit_date !='0000-00-00 00:00:00'
                    AND  input_records.stornirano='n'
                    GROUP BY clients.client_id";
            $obj = array(':wearehouse_id' => Session::get('wearehouse_id'),':sort_of_goods_id' => '2', ':type_of_goods_id' => $type_of_goods_id, ':goods_id' => $goods_id);
            $result = $this->model->get_values($sql, $obj);
            echo json_encode($result, JSON_NUMERIC_CHECK);
        } else {
            echo json_encode(array('logedIn' => 0), JSON_NUMERIC_CHECK);
    ...
    public function get_search_prijem_total()
    {
        ...
        if ($check_session['login'] == 1) {
            ...
            $sql = "SELECT
                    SUM(input_repromaterijal.neto) AS neto_total,
                    SUM(input_repromaterijal.kolicina) AS kolicina_total,
                    type_of_measurement_unit.measurement_name,
                    type_of_measurement_unit.measurement_unit
                FROM input_records
                INNER JOIN wearehouses ON (wearehouses.wearehouse_id = input_records.wearehouse_id)
                ...
                AND input_records.exit_date !='0000-00-00 00:00:00' AND input_repromaterijal.sort_of_goods_id='2' AND input_records.stornirano='n' ORDER BY input_records.input_id";
       ...
       // $params[':wearehouse_id'] = Session::get('wearehouse_id');
        ...
        if ($check_session['login'] == 1) {
            ...
            $sql = "SELECT
                ...
                INNER JOIN type_of_measurement_unit ON (type_of_measurement_unit.measurement_unit_id = goods.measurement_unit_id)
                INNER JOIN type_of_goods ON (type_of_goods.type_of_goods_id = input_records.type_of_goods_id)
                WHERE  " . $type_of_goods_id . " " . $goods_id . " " . $wearehouse . " " . $client_id . " " . $datum_od . " " . $datum_do . "  AND input_records.exit_date !='0000-00-00 00:00:00' AND input_records.sort_of_goods_id='2' AND input_records.stornirano='n' ORDER BY input_records.input_id";
    ...
    public function printExcel($get, $result, $suma){
        ...
        $xml = new ExcelWriterXML('Pregled prijema repromaterijala.xls');
        ...
        $sheet->writeString(3,5,'NAZIV DOBAVLJACA',$format5);
        $sheet->writeString(3,6,'ADRESA DOBAVLJACA',$format5);
        $sheet->writeString(3,7,'MESTO DOBAVLJACA',$format5);
    ...
    public function getPrijem($session_id=null){
        $input_id = $_GET['input_id'];
        $session = $session_id==null ? (string) $_GET['session_id'] : (string) $session_id;
        ...
        if( $check_session['login'] == 1){
            ...
            $result = $this->model->get_values($sql, array(":wearehouse_id"=>Session::get('wearehouse_id'), ":sort_of_goods_id"=>"1", ":input_id"=>$input_id));
            $result = $result[0];
