...
class Otprema_Merkantila_Api extends Controller
{
    ...
    public function get_goods_type(){
        Ajax::ajaxCheck();
        header('Content-Type: application/json');
        $sql = "SELECT * FROM type_of_goods WHERE goods_type='kukuruz tel-kel' || goods_type='kukuruz' || goods_type='psenica' || goods_type='suncokret' || goods_type='soja' || goods_type='jecam' || goods_type='uljana repica' || goods_type='sacma'";
    ...
    public function otprema_merkantila(){
        ...
        if( $check_session['login'] == 1){
            ...
            $output_merkantila_id = $this->model->set_values('output_merkantila', $goods_parametars);
            $document_br = $this->set_document_number();
            $date = new DateTime();
            $output_info= array(
                "document_br"          => $document_br,
                "output_merkantila_id" => $output_merkantila_id,
                "sort_of_goods_id"     => 1,
            ...
            $this->model->set_values('output_records',  $output_info);
        } else {
    ...
    public function enable_days(){
        ...
        $check_session = $this->check_logedIn($data->session_id); //checking if session exists
        if( $check_session['login'] == 1){
            $sql = "SELECT DATE_FORMAT(DATE(output_date),'%e-%c-%Y') as datum FROM output_records WHERE wearehouse_id= :wearehouse_id AND sort_of_goods_id= :sort_of_goods_id   GROUP BY DATE(output_date)";
    ...
    public function get_search_type(){
        ...
        if( $check_session['login'] == 1){
            ...
            $sql = "SELECT output_records.type_of_goods_id, type_of_goods.goods_type FROM output_records
                    ...
                    WHERE wearehouse_id= :wearehouse_id AND sort_of_goods_id= :sort_of_goods_id AND sort_of_goods_id= :sort_of_goods_id AND DATE(output_records.output_date)= :output_date AND output_records.stornirano='n' AND DATE(output_records.exit_date) != '0000-00-00'  GROUP BY type_of_goods_id";
       ...
       // print_r($data);return false;
        ...
        if( $check_session['login'] == 1){
            ...
            $sql = "SELECT goods.goods_id, goods.goods_name  FROM output_records
                    ...
                    AND output_records.sort_of_goods_id= :sort_of_goods_id
    ...
    public function get_search_prijemnica(){
        ...
        if( $check_session['login'] == 1){
            ...
            $type_of_goods_id = $data->output_type_id;
            $goods_id = $data->goods_id;
            $sql = "SELECT output_records.output_id, goods.goods_id, CONCAT(output_records.document_br, ' / ', YEAR(output_records.output_date)) As document_br ,goods.goods_name, clients.firm_name FROM output_records
                    INNER JOIN goods ON (goods.goods_id = output_records.goods_id)
                    INNER JOIN clients ON (clients.client_id = output_records.client_id)
                    WHERE output_records.wearehouse_id= :wearehouse_id AND output_records.sort_of_goods_id= :sort_of_goods_id AND DATE(output_records.output_date)= :output_date AND output_records.type_of_goods_id= :type_of_goods_id AND output_records.goods_id= :goods_id AND output_records.stornirano='n' ORDER BY output_records.output_id DESC";
            $obj = array(':wearehouse_id'=>Session::get('wearehouse_id'), ':sort_of_goods_id'=>'1', ':output_date'=>$output_date, ':type_of_goods_id'=>$type_of_goods_id, ':goods_id'=>$goods_id );
            $result = $this->model->get_values($sql,$obj);
            echo json_encode($result, JSON_NUMERIC_CHECK);
    ...
    public function prvo_merenje_merkantila($session_id=null){
        ...
        if( $check_session['login'] == 1){
            ...
            $sql='SELECT goods.goods_name,
                    ...
                    output_records.type_of_goods_id,
                  ...
                  AND output_records.sort_of_goods_id= :sort_of_goods_id AND output_records.stornirano="n" ORDER BY output_records.output_id DESC';
    ...
    public function drugo_merenje_merkantila($session_id=null){
        ...
        if( $check_session['login'] == 1){
            $sql='SELECT output_records.output_id, output_records.output_merkantila_id, output_merkantila.tara, type_of_goods.goods_type FROM output_records
            INNER JOIN output_merkantila ON(output_merkantila.output_merkantila_id = output_records.output_merkantila_id)
            INNER JOIN type_of_goods ON(type_of_goods.type_of_goods_id = output_records.type_of_goods_id)
    ...
    private function obracun_psenice($result){
        ...
        $this->_update_results($obj, $result['output_merkantila_id'], $result['output_id']);
    }
    ...
    private function _update_results($obj, $output_merkantila_id, $output_id){

        $table = 'output_merkantila';
        $where = 'output_merkantila_id='.$output_merkantila_id;
    ...
    public function select_last_input($session_id=null){
      ...
      //  print_r(Session::get('wearehouse_id'))l
        ...
        $sql='SELECT
                ...
                INNER JOIN goods ON (goods.goods_id = output_records.goods_id)
                INNER JOIN type_of_goods ON (type_of_goods.type_of_goods_id = output_records.type_of_goods_id)
                WHERE output_records.wearehouse_id= :wearehouse_id AND exit_date !="0000-00-00 00:00:00" AND output_records.sort_of_goods_id= :sort_of_goods_id
    ...
    public function select_odabrani_prijem(){
        ...
        if( $check_session['login'] == 1){
            $sql='SELECT
                ...
                INNER JOIN goods ON (goods.goods_id = output_records.goods_id)
                INNER JOIN type_of_goods ON (type_of_goods.type_of_goods_id = output_records.type_of_goods_id)
                WHERE output_records.wearehouse_id= :wearehouse_id  AND output_records.sort_of_goods_id= :sort_of_goods_id AND output_records.output_id= :output_id';
            $result = $this->model->get_values($sql, array(":wearehouse_id"=>Session::get('wearehouse_id'), ":sort_of_goods_id"=>"1", ":output_id"=>$data->output_id));
            echo json_encode($result);
