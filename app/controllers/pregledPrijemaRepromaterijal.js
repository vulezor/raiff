(function(){
    var pregledPrijemRepromaterijalController = function($scope, infoboxService, prijemRepromaterijalPregledFactory, errorService, mainService){
        $scope.search_data = {};
        ...
        $scope.show_prijem_repromaterijal = false;

        $scope.session_info = window.session_info;

        $('.prijem_repromaterijal_container').draggable({handle:'.prijem_repromaterijal_container_header'});
        ...
        //-------------------------------------------------------------------------------------------------------------
           ...
           console.log(prijemRepromaterijalPregledFactory);
       prijemRepromaterijalPregledFactory.get_search_good_type_admin().success(function(msg){
       ...
       $scope.getSearchGoodName = function(){
            prijemRepromaterijalPregledFactory.get_search_good_name_admin($scope.search_data).success(function(msg){
                $('.ajax_load_visibility').css({'visibility':'visible'});
                if(msg.logedIn !== 0 && msg.hasOwnProperty('logedIn')===false){
                    $('.ajax_load_visibility').css({'visibility':'hidden'});
                    console.log(msg);
        ...
        $scope.getSearchGoodWearehouses  = function(){
            prijemRepromaterijalPregledFactory.get_search_good_wearehouses_admin($scope.search_data).success(function(msg){
                $('.ajax_load_visibility').css({'visibility':'visible'});
                if(msg.logedIn !== 0 && msg.hasOwnProperty('logedIn')===false){
                    $('.ajax_load_visibility').css({'visibility':'hidden'});
                    console.log(msg);
          ...
          $scope.getGoodClient = function(){
            prijemRepromaterijalPregledFactory.get_search_good_client_admin($scope.search_data).success(function(msg){
                $('.ajax_load_visibility').css({'visibility':'visible'});
                if(msg.logedIn !== 0 && msg.hasOwnProperty('logedIn')===false){
                    $('.ajax_load_visibility').css({'visibility':'hidden'});
                    console.log(msg);
        ...
        $scope.getTotalOfResult = function(){
            prijemRepromaterijalPregledFactory.get_search_prijem_total_admin($scope.search_data).success(function(msg){
                $('.ajax_load_visibility').css({'visibility':'visible'});
                if(msg.logedIn !== 0 && msg.hasOwnProperty('logedIn')===false){
                    $('.ajax_load_visibility').css({'visibility':'hidden'});
                    console.log(msg);
       ...
       /* $scope.logout = function(){
        ...
        $scope.getExcell = function(){
            ...
            window.open(mainService.domainURL()+'pregled_prijema_repromaterijal_admin_api/getExcell?'+$scope.link,'_blank');
        };

        //----------------------------------------------------------------------------------------------------------------------------------

        $scope.showPrijemRepromaterijalContainer = function(){
            $scope.show_prijem_repromaterijal = !$scope.show_prijem_repromaterijal;
        };
        ...
        $scope.stornirajPrijemnicu = function(){
            ...
            prijemRepromaterijalPregledFactory.storniraj_dokument(obj).success(function(msg){
    ...
    pregledPrijemRepromaterijalController.$inject = ['$scope', 'infoboxService', 'prijemRepromaterijalPregledFactory', 'errorService', 'mainService'];
    ...
    //***************************************************************************************************************************************************************
     ...
     var pregledPrijemaRepromaterijalTable = function( DTOptionsBuilder, DTColumnBuilder, usersFactory, $scope, $timeout, $resource, $compile, mainService, prijemRepromaterijalPregledFactory, errorService) {
        ...
        vm.dtOptions = DTOptionsBuilder
            .fromSource(mainService.domainURL()+'pregled_prijema_repromaterijal_admin_api/empty_load')
        ...
        function reloadData() {
            ...
            vm.dtInstance.changeData(mainService.domainURL()+'pregled_prijema_repromaterijal_admin_api/get_search_prijem/?'+$scope.$parent.link);
        }
        ...
        function printPrijemnica(id){
          ...
          prijemRepromaterijalPregledFactory.getPrijem({'input_id':id.input_id}).success(function(msg){
                console.log(msg);
        ...
        function actionsHtml (data, type, full, meta) {
            ...
            var t='<i class="fa fa fa-print btn btn-primary btn-xs" style="margin-top:0px" title="Prijemnica" data-ng-click="prijemShowCase.printPrijemnica(prijemShowCase.prijemResult[' + data.input_id + '])"></i>';
            if( $scope.$parent.session_info === 'Administrator' || $scope.$parent.session_info === undefined ){
                t +='<i class="fa fa fa-exclamation-triangle btn btn-danger btn-xs" style="margin-top:0px;margin-left:5px" title="Storniraj prijemnicu ' + data.input_id + '" data-ng-click="prijemShowCase.predStorniranje(prijemShowCase.prijemResult[' + data.input_id + '])"></i>';
            }
            return t;
        }
    };
    ...
    var prijemRepromaterijalController = function($scope, mainService, _, $filter, prijemRepromaterijalPregledFactory, errorService, errorService_second, clientsFactory){
        $scope.insert_data = {};
        $scope.goods_type = {};
        $scope.goods_name = {};
        $scope.clients = [];
        ...
        $scope.setResult = function(){
            prijemRepromaterijalPregledFactory.insertRepromaterijal($scope.insert_data).success(function(msg){
                if(msg.logedIn !== 0 && msg.hasOwnProperty('logedIn')===false){
                    console.log('logedin');
        ...
        $scope.selectLastMeasurement = function(){
            prijemRepromaterijalPregledFactory.selectLastInput().success(function(msg){
                console.log(msg);
                if($.isEmptyObject(msg)===false){
    ...
    prijemRepromaterijalController.$inject = ['$scope', 'mainService', '_', '$filter', 'prijemRepromaterijalPregledFactory','errorService', 'errorService_second', 'clientsFactory'];
